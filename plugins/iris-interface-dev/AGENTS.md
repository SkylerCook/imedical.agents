...
`iris-interface-dev` 提供 IRIS 接口开发的文档优先能力：接口文档落盘、结构化抽取、字段匹配诊断、接口开发计划、编码交接和离线审查。

本插件借鉴 Java 接口开发插件的流程思想：先明确文档和字段事实，再做范围/计划门禁，最后把实现交给专项编码能力。IRIS/ObjectScript、CSP、JavaScript、HISUI 编码规则、代码审查、上传、编译、部署和远端验证必须复用 `coding-iris-plugin`。
