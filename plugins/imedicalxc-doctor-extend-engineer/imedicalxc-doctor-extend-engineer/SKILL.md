...
使用本 skill 附带的 `install-deps.py` 自动发现并安装依赖链。
脚本位于本 SKILL.md 同级 `scripts/` 目录下，执行时请基于
SKILL.md 所在目录拼接脚本路径（AI 可从已加载的 skill 文件路径推导）。

示意（`<skill-dir>` 替换为 SKILL.md 所在目录）：
```bash
# 首次全量安装（全部传递依赖）
python <skill-dir>/scripts/install-deps.py <target-artifact-id>

# 增量安装（仅安装源码比 jar 新的模块，日常使用）
python <skill-dir>/scripts/install-deps.py <target-artifact-id> --changed-only

# 只看依赖顺序，不执行安装
python <skill-dir>/scripts/install-deps.py <target-artifact-id> --dry-run
```
