...
3. 新模板编码 — 如 `DHCDocIPBookPrt_BJHDFY`
4. 新模板名称 — 如 `海淀妇幼保健院-住院证打印`
...
```bash
cd workdir
find . -type f | sed 's|^\./||' > /tmp/files.txt
/c/Windows/System32/tar.exe -cf ../输出模板.xlsx --format=zip -T /tmp/files.txt
```
...
```bash
# 先导入主模板
curl -F "file=@DHCDocIPBookPrt_XXX_主模板.xlsx" \
  ".../sys/import?importType=insertOrUpdate&code=hos_print_template"

# 再导入扩展模板
curl -F "file=@DHCDocIPBookPrt_XXX_扩展模板.xlsx" \
...
每次生成后应产出：
1. `DHCDocIPBookPrt_XXX_主模板.xlsx` — 导入code: `hos_print_template`（**必须**）
2. `DHCDocIPBookPrt_XXX_扩展模板.xlsx` — 导入code: `hos_print_template_extend`（如参考模板中有扩展模板则**必须**，仅有主模板时此项为可选）
3. 字段映射文档（布局字段→API字段对照表）
4. 修正清单（Sheet4类型/名称修正明细）
5. 缺失字段清单（需后续补充的Sheet4行）

## 参考文件

- 工作流详细文档：`D:/#Git/Git-HIS_4/打印模板设计工作流.md`
- 现有模板参考：`D:/#Git/Git-HIS_4/住院证打印模板/`
- 生成脚本模板：`D:/#Git/Git-HIS_4/住院证模板_HDFYBJ/build_v7.js`
- Sheet4修正脚本：`D:/#Git/Git-HIS_4/住院证模板_HDFYBJ/fix_sheet4.js`
