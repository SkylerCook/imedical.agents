...
- 前端 `.csp` / `.js` / `.css` 必须符合 `iris_project_profile.md` 的 canonical `utf8`；每个文件改造前后执行 UTF-8 字节检查，字节检测是最终门禁。旧模式只按 coding-iris 兼容边界读取。
...
- 后端 `.cls` 文件编码为 UTF-8。
- 当前前端文件编码统一为 UTF-8；历史 `standard-gb2312` 只有在用户明确指定旧工程时才作为例外验证。
