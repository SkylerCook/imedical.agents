...
先读取插件 `../../AGENTS.md` 的适用硬约束及 `../../references/cure-form-delivery-workflow.md` 的“工作目录”“自动部署 / 手动部署”；预览、多 Map 或恢复演练按该文件路由补读。已有内容可在未变化时复用。部署选择与授权沿用该流程的唯一约定，不重复要求用户选择；写入结果未知时停止并只读核实，不重试。
...
3. `apply` 默认 dry-run；真实写入必须具有覆盖当前对象、内容和操作范围的明确用户授权；授权沿用交付流程约定，并传 `--confirm-write --operator ... --reason ...`。
4. 通道尚未明确且影响执行时，说明完整事务包和轻量 SQL 覆盖的适用范围；已有明确选择时直接沿用。完整事务包只调用 `DHCDoc.Cure.AI.CureFormDeploy` 专用方法；选择 `lightweight-sql` 时先完整阅读 `../../references/cure-form-sql-cover.md`，仅允许单个已有独占模板 content 的固定参数化覆盖，60/120 秒预算，不伪造服务器 operation ID。静态资源上传编译委托 `coding-iris-plugin/iris-deploy`，用户可自行上传。
