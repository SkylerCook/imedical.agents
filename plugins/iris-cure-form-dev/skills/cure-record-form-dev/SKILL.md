...
8. 生成部署包前先通过 `preview`/`preview-run`/`preview-check` 的完整资源、CSS 依赖、Console、HISUI 初始化和九档宽度门禁；新建表单再用 `interaction-prepare`/`interaction-check` 完成人工输入、计算、联动和去重验证，`plan` 必须携带交互凭证。
9. 部署后完成人工 CR 保存、重开、回显、打印以及 `SaveCureRecord`、`CureExpJsonStr`、`MapID` 验证。用户明确反馈通过可作总体确认；Agent 自测必须逐项记录。批量脚本化交互执行前必须申请用户明确确认，失败时停止交付且不自动回滚。
