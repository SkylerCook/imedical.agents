...
3. 完整 HTML 从目标 `cure_form_profile.md` 或现有页面解析并加载 HISUI、jQuery、locale、公共基础样式、公共响应式样式和规格声明的独立业务 CSS；缺少任一 canonical 资源时停止，片段仍只包含根 div。
...
6. 新建表单先运行 `interaction-prepare --stage pre-deploy` 生成并展示人工清单；用户亲自测试时以其明确“已测试通过”反馈形成 `user-attested` 凭证，Agent 在本地完整预览自测时以 `agent-manual` 逐项记录。任何批量脚本化点击、输入或选择必须先申请用户明确确认，v1 不接受自动模式。
7. `interaction-check` 通过后才允许为 `expectedVersion=NEW` 的表单执行 `plan --interaction-verification ...`。部署后再生成清单并验证 CA 保存、重开、回显和打印；失败时停止交付，不自动回滚。
8. `expectedTemplateCount` 存在时，生成的 fragment、部署模板和 Map composition 数量必须全部严格相等；JavaScript 数量按实际业务逻辑计算，不得强制等于模板数。
9. 规格声明 `stylesheets[]` 时，生成对应 CSS、独立预览静态 link、表单级 JS 加载器及 `stylesheet` 部署资源，并验证内容哈希缓存版本。
10. 新增部署静态资源 basename 必须语义明确、使用 camelCase 且不超过 24 个字符。业务 `moduleId` / `moduleName` 超长时保留稳定业务标识，另行声明短资源名；引用路径和部署 basename 必须一致。允许 `Struct`、`Func`、`Assess` 及公认临床缩写，禁止点分命名和 `ass` 等含糊缩写。
