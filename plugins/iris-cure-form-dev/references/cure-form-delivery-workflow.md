...
插件 AGENTS.md 的适用安全门禁与兼容契约始终有效；本文件按下表读取命中章节，不要求每次从头通读。已持有且未变化的内容可复用；阶段扩展时补读对应章节。默认步骤可合并或重排，但不得越过规格批准、验证和写入授权。

| 任务信号 | 读取章节 |
|---|---|
| 创建或修改任务产物 | 工作目录 |
| 生成或验证预览、调整布局 | 只读 vendor 挂载、视觉质量 |
| 准备部署包、手动交付或远程写入 | 自动部署 / 手动部署；并加载 cure-form-deploy skill 的部署规则 |
| 多 Map、并行写入或复用验证凭证 | 并行质量与效率 |
| 用户明确要求清空/恢复演练 | 显式恢复演练 |

局部布局任务不因读取本插件而自动进入部署、灰度收尾或恢复演练。部署相关硬约束在准备部署时全部加载，不能以精简模式跳过。
...
部署方式、通道、发布策略和静态资源方式是四个独立选择。先核对用户当前要求及会话内已有明确选择；只有未明确且影响下一步执行时，使用 `deployment-options` 展示相关选项并补齐决定，不反复展示全部菜单。项目默认值用于准备计划，不视为远程写入授权。

真实写入前核对已有明确用户授权是否覆盖当前对象、内容和操作范围；覆盖时直接沿用，不仅因进入 apply 阶段再次询问。授权缺失、目标或内容超出原授权、共享影响扩大、基线冲突或写入结果未知时停止，完成可安全执行的只读核实后再请求必要决定。未知结果不得以再次确认替代状态核实。`--confirm-write`、操作者、原因、版本/哈希门禁仍须保留；部署授权不自动覆盖 rollback、清理、恢复演练或患者数据写入。
