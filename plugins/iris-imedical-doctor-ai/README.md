...
现有参考进一步覆盖快捷入口与业务校验分层、传统页面服务抽取等价性、配置驱动引入、实际加载/控件事件验证和文书摘要语义；脱敏场景见 verification.md。场景是测试设计输入，不表示已有通用运行时测试器。升级仅更新指导资料，无基类、业务源码或连接配置迁移；已部署副本在发布后通过既有更新流程同步。
