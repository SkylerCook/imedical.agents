...
# 导出 JS 文件（自动添加 imedical/web/ 前缀）
node .agents/plugins/coding-iris-plugin/scripts/iris-tools/export.js scripts/Alloc.ExaBorRoom.hui.js

# 导出 CSP 文件（自动添加 imedical/web/csp/ 前缀）
node .agents/plugins/coding-iris-plugin/scripts/iris-tools/export.js alloc.exaborroom.hui.csp

# 使用完整路径导出
node .agents/plugins/coding-iris-plugin/scripts/iris-tools/export.js imedical/web/scripts/test.js

# 自定义输出目录和命名空间
node .agents/plugins/coding-iris-plugin/scripts/iris-tools/export.js DHCDoc.AI.KBase my-output DHC-APP
...
2. **JS 文件**：以 `.js` 结尾、包含 `.hui.js`，或位于 `scripts/` 目录下
   - 如果不存在则自动添加 `imedical/web/` 前缀
   - 示例：`scripts/test.js` → `imedical/web/scripts/test.js`

3. **CSP 文件**：以 `.csp` 结尾或位于 `csp/` 目录下
   - 如果不存在则自动添加 `imedical/web/csp/` 前缀
   - 示例：`test.csp` → `imedical/web/csp/test.csp`
...
# 指定命名空间
node .agents/plugins/coding-iris-plugin/scripts/iris-tools/compile.js DHCDoc.Util.String DHC-APP
```
...
- serverUrl（可选，默认使用 `/imedical/web/csp/websys.Broker.cls`）
...
- `--url`：自定义 serverUrl 路径（可选，会自动添加 `/imedical/web/` 前缀）
