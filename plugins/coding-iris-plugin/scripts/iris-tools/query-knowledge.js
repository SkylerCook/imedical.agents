...
function search({context,query,sourceId,limit=20}) {
  ...
  if(sourceId) {
    ...
    if(!state.snapshot) throw new Error('No current project menu snapshot; run iris-menu-sync first');
    for(const [file,text] of Object.entries(render(state.snapshot))) add(text,{kind:'project-menu',sourceId,
  ...
  return {matches,limit,reachedLimit:matches.length===limit};
}
function main(argv) {
