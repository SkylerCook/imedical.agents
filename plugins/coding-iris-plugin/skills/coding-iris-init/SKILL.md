...
1. 解析工作区：
   - 先调用 capability 包 `scripts/lib/WorkspaceContext.psm1` 或 `scripts/lib/workspace-context.js` 解析 `WorkspaceRoot`、`ContextRoot`、`CapabilityRoot`、`SourceRoot` 和 `GitRoot`；不得自行向父目录或 sibling 扫描 `.agents`。
   - standard 模式下三类根保持现有同目录行为；workspace-overlay 模式下，配置、规则、记忆和工作目录只写 `ContextRoot`，插件、模板和 vendor 从 `CapabilityRoot`（或 `ContextRoot` 的 shared Junction）读取。
2. 检查目标工程：
   - 是否存在 `AGENTS.md`。
   - 是否存在 `.mcp.json`。
...
5. 初始化 IRIS 开发主力脚本配置：
   ...
   - SFTP 默认禁用。新建配置可选择 `sftp.runtime: vendor`，使用 CapabilityRoot 内置运行时；先按 `vendor/sftp-server/README.md` 准备 Python 依赖及可信主机密钥。已有可识别的 legacy 启动路径在更新时自动迁移 vendor；显式 custom 保持不变。`knownHosts`、`keyFile`、`allowRemoteCommands` 分别映射对应私有环境字段，不写入可复用文档。
...
6. 生成 thin-index：
   - 先执行 DryRun：
     ...
     ```
   - **若 DryRun 全部 skipped**：补充提示"所有 thin-index 已存在且指向正确插件路径，无需更新。如需强制重建，使用 `-Force` 参数。"然后跳到步骤 7。
...
8. 验证：
   ...
   - standard 模式的 `.agents/.git/info/exclude` 已包含生成层忽略规则：`/config/`、`/memory/`、`/rules/`、`/skills/`、`/scripts/`；workspace-overlay 的 `ContextRoot` 不要求存在 `.git`，本地生成层由工作区仓库自己的忽略规则管理。
...
`iris_project_profile.md`、`project-env.json`、thin-index 和复制到 `.agents/scripts/` 的本地脚本属于目标工程本地生成层。standard 模式初始化或重建索引后，必须确认这些路径由 `.agents/.git/info/exclude` 隐藏，不要把生成层忽略规则写进 `.agents/.gitignore`。workspace-overlay 模式则由 WorkspaceRoot 所属仓库的忽略规则管理，禁止要求 `ContextRoot/.git/info/exclude`。
