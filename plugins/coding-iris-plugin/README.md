...
- 标版需求闭环：`skills/iris-demand-entry/SKILL.md` 从工作区或指定提交实际 diff 生成可复制需求文本，可选 `--excel`（兼容 `--Excel`）生成 29 列 BOSS 导入表。用户录入并回填编号/最终标题后生成提交消息，待提交代码复用 iris-demand-commit；历史提交不自动改写。用户模式为 `--text / --bind / --plan / --commit / --help`，历史来源使用 `--rev` 或 `--range`。四个写入入口会拒绝 `.agents/` 或能力包源码内的输出路径，包括目录链接别名；旧位置的输入仍可读取。命令与草稿见 [操作协议](references/standard-demand-entry.md)。已部署工程更新能力包并重建 enabled 插件 thin-index 后使用，无配置迁移或旧入口删除。
