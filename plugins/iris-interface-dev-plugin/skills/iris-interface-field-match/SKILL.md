...
python .agents/plugins/iris-interface-dev-plugin/scripts/iris-interface-field-match.py `
  --parsed docs/output/iris-interface/<doc-name>/parsed.json `
  --project-root .
```

如目标项目已有本地人工反馈，可额外指定 JSON 文件：
...
`--feedback` 只能读取目标项目本地文件，不写回插件仓库，也不自动提升为通用规则。

## 输出

固定写入 `parsed.json` 同目录：

- `field-match.json`
- `field-match.md`

匹配结果区分 `builtin-rule`、`local-feedback`、`low-confidence-candidate` 和 `unmatched`。对话里只汇报路径、数量和最高优先级人工确认项，不打印完整字段清单。
