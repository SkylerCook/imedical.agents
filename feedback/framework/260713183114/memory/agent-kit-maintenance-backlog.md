...
- 下一项合适需求必须从 Step 0 即采用 `multi-agent`：先建 manifest、记录实际时间、声明文件所有权和一次性远程授权，所有最终修改后再执行独立 Verifier 与机械校验。
- 结合 `#6096150` 脱敏串行回溯、`#6097879` 首次真实实战与下一次标准化实战后，再决定 i18n 样板是否定型；若要新增通用 `standard-change`、`review-test-release`、`bugfix` workflow 或通用 Agent，至少再补一个非 i18n 或明显不同任务形态样本。
