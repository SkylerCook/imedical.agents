...
- 表单输入使用 `combobox`、`validatebox`、`lookup`、`datebox`、`numberbox` 等 HISUI 标准控件，并明确初始化所有权：
  - 声明式初始化：原始元素使用 `hisui-<plugin>` class 和 `data-options`，交给 HISUI parser 自动初始化；页面 JavaScript 不得再次调用同一控件的初始化方法。
  - JavaScript 初始化：原始输入使用普通 `textbox`（或目标工程未被 parser 扫描的基础 class），由页面初始化函数只调用一次 `$(selector).<plugin>(options)`。
  - 静态本地 `combobox` 数据是否需要显式 `url: ""` 取决于目标 HISUI 版本和项目扩展；本次验证版本需要 `url: ""` + `data: [...]`。异步数据仍在接口返回后调用 `loadData`。
- 下拉为空或样式异常时，优先检查 `hisui-<plugin>` 自动解析与 JavaScript 手工初始化是否并存，再检查 `url/data/loadData` 和接口返回。
...
- 对由 JavaScript 初始化的 HISUI 控件，检查原始 DOM 是否仍带 `hisui-<plugin>` 自动解析 class；同一控件只能保留一种初始化所有权。
