...
- `hisui-<plugin>` class 会被 parser 自动初始化；如果页面还会调用 `$(selector).<plugin>(options)`，原始元素应改用不被 parser 扫描的基础 class（常见为 `textbox`），避免重复初始化。
- 本地静态 `combobox` 的 `url/data` 行为可能受 HISUI 版本或项目扩展影响；本次验证版本使用 `url: ""` + `data: [...]`，异步数据使用 `loadData`。以目标页面实际加载版本为准。
...
.agents/vendor/hisui/
├── dist/
│   ├── js/jquery.hisui.js        ← 主 JS，控件 API 定义在此，25,148 行
│   └── css/                      ← 样式与图标资源
│       ├── themes/
│       │   ├── default/
│       │   └── metro-blue/
...
| **combobox** | 17322 | 17446 | → combo | 下拉列表选择 |
...
| **menutree** | 24641 | 24703 | — | 菜单树导航，含搜索和折叠能力 |
...
| 名称 | 行号 | 说明 |
|---|---:|---|
| `parser` | 500 | HTML 解析器，扫描 `hisui-<plugin>` class 并自动初始化控件 |
| `_remove` | 3897 | DOM 移除辅助 |
| `$.hisui.styleCodeConfig` | 345 | 主题风格配置（default / pure / vben / lite / lightblue） |
| `$URL` | 24924 | 默认 Broker URL：`websys.Broker.cls` |
| `Level` | 24926 | 日志级别枚举（DEBUG / INFO / WARN / ERROR） |
| `logger` | 24987 | 控制台日志实例 |
...
查 menutree：         offset=24641 limit=120
```
