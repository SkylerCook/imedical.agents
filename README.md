...
- `iris-demand-commit` 为已完成的标版/项目需求生成仓库级方案型提交信息：标版首行使用简短菜单/功能摘要、第三行保留完整需求，并在提交前强制 `pull --ff-only`；新计划批量读取 Git 状态，获授权后可用一次 `apply --verify` 完成提交与校验。项目兼容无 upstream 的纯本地仓库，push 始终另行授权。
