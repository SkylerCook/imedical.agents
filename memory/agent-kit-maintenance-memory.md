...
- `coding-iris-plugin` 当前 v0.13.2：新增共享知识检索、项目菜单同步及部署兼容；保留 standard 需求录入与提交衔接；前端 canonical 编码为 `utf8`，实际字节检测是最终门禁；真实 GB2312/mixed/unknown 不自动转码，明确 backend-only Overlay 为 `N/A (backend-only)`。CLS 仅提供最小改动提示与可选自检，不增加编译证据或提交门禁。
