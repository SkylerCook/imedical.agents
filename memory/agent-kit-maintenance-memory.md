...
- `plugins/iris-cure-form-dev/` 负责 CA 治疗评估与 CR 治疗记录的文档语义适配、Excel 多模板边界报告与有序生成、响应式改造及受控部署编排；Map 总入口与模板逻辑均使用运行时外部 JS 路径，部署路径单独声明，无逻辑模板保持空引用；默认由宿主管理分模板生命周期，已验证宿主不可靠时可显式启用 `aggregateTemplateInit=true`，由总入口延迟且幂等调度实际业务模块；响应式门禁覆盖 HISUI `i-label-box` / `m-label-box` radio 配对和旧 WebView fallback；通用文档解析委托 `extract-doc`，IRIS/HISUI 与静态资源部署复用 `coding-iris-plugin`，空 `MapType` 病理模板不进入流程。
