...
- `iris-cure-form-dev` v0.3.2 使用 gate v2 与 canonical Chromium runner，旧/人工浏览器凭证不再接受；新 profile 默认使用 `.agents/vendor/hisui/`，现有项目更新后保留本地六资源配置并重新生成凭证。`common-migrate` 还需项目自有 `cure-form-common-migration-config/v1`，插件不再内置业务种子。
