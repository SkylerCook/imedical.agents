...
- `coding-iris-plugin` 当前 v0.10.2：前端 canonical 编码为 `utf8`，实际字节检测是最终门禁；真实 GB2312/mixed/unknown 不自动转码，明确 backend-only Overlay 为 `N/A (backend-only)`。CLS 仅提供最小改动提示与可选自检，不增加编译证据或提交门禁。
...
- `iris-imedical-doctor-ai` v0.1.2 提供医生站 AI 开发方法，补齐入口分层、共享页面等价性、运行时接入及摘要检查；工程、原型与接口按任务核实，菜单/wiki 接入尚未实现。其余插件职责与入口见仓库 README，不在本摘要重复目录清单。
