...
- 部署保护 v0.10.0：本机专项与独立真实资源验证已通过；Windows/macOS/Linux、Node 22/24 矩阵已配置，待发布后获取 CI 结果。不同后端导出格式仍按逐环境确认，未知格式不自动归一化。当前工程副本为未发布开发版，正式发布后再按正常更新器收敛。无后台监控、远端锁与自动回滚，本期明确不建设。
