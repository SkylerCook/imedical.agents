...
2026-09-14：将本会话已验证的 Atelier CSP 编译方式固化到 compile-csp.js，并入尚未发布的 coding-iris-plugin 0.8.0。支持一次批量编译、纯本地计划、显式执行、超时/错误/重定向停止和耗时输出；同步替换旧编译指引。只编译明确目标，包括 show.csp，不自动扩展父页面。新脚本使用本地 HTTP 模拟服务验证，不重复部署业务文件；此前真实 API 编译验证与本次脚本离线验证分开记录。

2026-09-14：coding-iris-plugin 0.8.0 纳入自维护 SFTP vendor、带哈希依赖锁和显式配置迁移；五个下游仅扩展依赖兼容范围。修复 Windows 忽略/路径边界、SHA-256 原子上传、流读取/命令超时及主机密钥优先级。用户授权的 JS、CSS 各一份通过真实 MCP 上传与独立 SFTP 回读一致性验证，未留下远端临时文件；本地原工具及业务配置未变。Windows Python/MCP 与 PS5.1/PS7 迁移测试通过；非 Windows 矩阵等待 CI，HIS 业务验收独立。未提交、未推送、未更新业务副本。
